...
- Ist enger gefasst (nur `example.lan`) und damit unkritisch für andere Domains im Netz.
...
```
address=/.example.lan/::0
```

Setzt für **alle Subdomains von `example.lan`** den AAAA-Record auf `::` (IPv6-Null-Adresse). Dnsmasq antwortet bei AAAA-Anfragen direkt damit, ohne Upstream zu fragen.
...
Macht dasselbe für die Apex-Domain `example.lan` selbst (ohne Subdomain-Prefix).
...
```bash
dig example.lan AAAA           @192.168.1.10   # → :: oder NODATA
dig heimdall.example.lan AAAA  @192.168.1.10   # → NODATA (durch Pi-hole-Regex)
                                                  #   oder :: (durch dieses Snippet)
dig heimdall.example.lan A     @192.168.1.10   # → 192.168.1.20 (unverändert)
```

## Anpassung für andere Domains

Statt `example.lan` einfach die eigene Domain einsetzen:
